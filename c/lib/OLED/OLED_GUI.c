...
UBYTE FAN_MODE  ;
void POE_HAT_Display(UBYTE *WIFI_IP,UBYTE *Eth0_IP,UWORD TEMP, uint32_t FREE_MEM, UBYTE FAN_TEMP){
    UBYTE t[10];
    GUI_Clear();
    UWORD yLineSize = 10;
    UWORD xNet = 11;
    ...
    if(Eth0_IP[0] != 0){
        GUI_DisString_EN(0 , 0, "E:", &Font12, FONT_BACKGROUND, WHITE);
        GUI_DisString_EN(xNet , yNet, Eth0_IP, &Font12, FONT_BACKGROUND, WHITE);
    }else if(WIFI_IP != 0){
        GUI_DisString_EN(0 , 0, "W:", &Font12, FONT_BACKGROUND, WHITE);
        GUI_DisString_EN(xNet , yNet, WIFI_IP, &Font12, FONT_BACKGROUND, WHITE);
    }else{
        GUI_DisString_EN(0 , 0, "E:", &Font12, FONT_BACKGROUND, WHITE);
        GUI_DisString_EN(xNet , yNet, Eth0_IP, &Font12, FONT_BACKGROUND, WHITE);
    }
    ...
    UWORD xTemp = 0;
    UWORD yTemp = yLineSize;
    GUI_DisString_EN(xTemp , yTemp, "T:", &Font12, FONT_BACKGROUND, WHITE);
    // printf("TEMP = %d\r\n",TEMP);
    if( TEMP/1000>=100){
    ...
    UWORD xTempValue = xTemp + 11;
    GUI_DisString_EN(xTempValue , yTemp, t, &Font12, FONT_BACKGROUND, WHITE);

    UWORD xFan = xTempValue + 30;
    GUI_DisString_EN(xFan , yTemp, "Fan:", &Font12, FONT_BACKGROUND, WHITE);
    ...
    UWORD xFanValue = xFan + 25;
    if(FAN_MODE == 1){
        GUI_DisString_EN(xFanValue , yTemp, "1", &Font12, FONT_BACKGROUND, WHITE);    
    }else{
        GUI_DisString_EN(xFanValue , yTemp, "0", &Font12, FONT_BACKGROUND, WHITE);
    }
    ...
    for(UWORD i=0;i<10-s+1; i++){
	    mem[i] = mem[i+s+1];
    }
    
    for(UWORD i=10-s-1;i<=10; i++){
	    mem[i] = ' ';
    }
    GUI_DisString_EN(xMem , y, mem, &Font12, FONT_BACKGROUND, WHITE);
